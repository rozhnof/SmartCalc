...
void CreditCalcModel::setInput(CreditCalculatorInput input) { _input = input; }

void CreditCalcModel::Calculate(Service::CreditPaymentsType type) {
  _output = service.GetCreditCalculationResult(_input, type);
}

double CreditCalcModel::getTotalPayment() { return _output.totalPayment; }

double CreditCalcModel::getOverpayment() { return _output.overpayment; }
