...
View::View() : _calc(new CalculatorUI), _creditCalc(new CreditCalculatorUI), _depositCalc(new DepositCalculatorUI), _graph(new GraphUI), _winUi(new WinUI), _macUi(new MacUI) {
    _currentTab = _calc;
    _platform = _winUi;
...
void View::switchToCalc() {
    switchTo(_calc);
}

void View::switchToCreditCalc() {
    switchTo(_creditCalc);
}

void View::switchToDepositCalc() {
    switchTo(_depositCalc);
}

void View::switchToGraph() {
    switchTo(_graph);
}
