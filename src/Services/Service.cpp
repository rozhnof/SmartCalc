...
double Service::GetCalculationResult(const std::string &postfix,
                                     const double &x) {
...
GraphOutput Service::GetGraphResult(const GraphInput &input) {
  ...
  for (int i = 0; i < input.countPoints; i++) {
    ...
    output.yPoints.push_back(
        GetCalculationResult(input.postfix, output.xPoints[i]));
...
CreditCalculatorOutput Service::GetCreditCalculationResult(
...
DepositCalculatorOutput Service::GetDepositCalculationResult(
  ...
  for (auto itDay : input.daysInPeriod) {
    ...
    if (itDay.isPaymentDay) {
      ...
      output.generalList.push_back(std::make_tuple(itDay, operationType,
                                                   accumulatedInterestForPeriod,
                                                   output.totalAmount));
    ...
    for (auto it : itDay.topUpList) {
      ...
      output.generalList.push_back(
          std::make_tuple(itDay, "Пополнение", it, output.totalAmount));
    }

    for (auto it : itDay.takeOffList) {
      output.totalAmount -= it;
      output.generalList.push_back(
          std::make_tuple(itDay, "Снятие", it, output.totalAmount));
    }
...
CreditCalculatorOutput Service::AnnuityLoan(
    const CreditCalculatorInput &input) {
  CreditCalculatorOutput output = {};

  double monthlyInterestRate = input.interestRate / 12 / 100;
  double tmp = pow(1 + monthlyInterestRate, input.creditTerm);
  double monthlyPayment =
      input.creditSum * monthlyInterestRate * tmp / (tmp - 1);
  ...
  for (int i = 0; i < input.creditTerm; i++) {
    ...
    output.monthlyBodyPayments.push_back(monthlyPayment -
                                         output.monthlyPercentPayments[i]);
    creditSum -= output.monthlyBodyPayments[i];
  }
  return output;
}

CreditCalculatorOutput Service::DifferentiatedLoan(
    const CreditCalculatorInput &input) {
  CreditCalculatorOutput output = {};
  ...
  for (int i = 0; i < input.creditTerm; i++) {
    output.monthlyPercentPayments.push_back(
        (input.creditSum - (monthlyBodyPayment * (i))) * monthlyInterestRate);
    output.monthlyPayments.push_back(monthlyBodyPayment +
                                     output.monthlyPercentPayments[i]);
